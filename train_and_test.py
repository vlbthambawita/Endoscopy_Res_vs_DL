...
parser.add_argument("--input_img_size", type=int, default=32, help="Input image size to the network")
parser.add_argument("--rescaled_to", type=int, default=64, help="Rescaling to this image size before rescaling back to input image size")
parser.add_argument("--stop_lr", type=float, default=0.000001, help="Learning rate for training")

# Action handling 
parser.add_argument("--num_epochs", type=int, default=1000, help="Numbe of epochs to train")
...
parser.add_argument("--val_fold", type=str, default="split_0", help="Select the validation fold", choices=["split_0", "split_1"])
...
parser.add_argument("--test_checkpoint", default="", help="A path to a checkpoint file")
parser.add_argument("--model", default="resnet152", choices=["resnet152", "densenet161"])
...
py_file_name = opt.py_file.split("/")[-1] + "_{}_{}_{}".format(opt.model, opt.input_img_size, opt.rescaled_to) # Get python file name (soruce code name)
...
def prepare_data():
    ...
    data_transforms = {
        'train': transforms.Compose([
            transforms.Resize(opt.rescaled_to),
            transforms.CenterCrop(opt.rescaled_to),
            transforms.Resize(opt.input_img_size),
...
def train_model(model, optimizer, criterion, dataloaders: dict, scheduler, best_acc=0.0, start_epoch = 0, stop_lr=0.0):
    ...
    for epoch in range(start_epoch , start_epoch + opt.num_epochs ):
        ...
        if current_lr <= stop_lr:
            print("stopped training with LR=", current_lr)
...
def run_train(retrain=False):
    ...
    if retrain:
        ...
        acc = checkpoint["acc"]
        train_model(model,optimizer,criterion, dataloaders, scheduler, best_acc=acc, start_epoch=start_epoch, stop_lr=opt.stop_lr)

    else:
        train_model(model,optimizer,criterion, dataloaders, scheduler, best_acc=0.0, start_epoch=0, stop_lr=opt.stop_lr)
...
def check_model_graph():
    ...
    summary(model, (3, opt.input_img_size, opt.input_img_size)) # this run on GPU
    model = model.to('cpu')
  
    print(model)
    #dummy_input = Variable(torch.rand(8, 3, opt.input_img_size, opt.input_img_size))
...
def test_model():
    ...
    with open("%s/RESCALING_%s_evaluation_on_upscaled_from_%s.csv" % (opt.out_dir, py_file_name, opt.rescaled_to), "w") as f:
        ...
        f.write("Accuracy: %s\n" % mtc.accuracy_score(y_true, y_predicted))
